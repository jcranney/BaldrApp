...
def runtime_dynamic_state_from_control(dynamic_state, sim_control):
    ...
    state["phase_enabled"] = bool(dynamic_state.get("phase_enabled", False) and sim_control["phase_enabled"])
    state["ao_enabled"] = bool(
        dynamic_state.get("ao_enabled", False)
        and sim_control["ao_enabled"]
        and state["phase_enabled"]
    )
    state["scint_enabled"] = bool(dynamic_state.get("scint_enabled", False) and sim_control["scint_enabled"])
    ...
    try:
        ...
        if cmd == "mode":
            if len(tokens) != 2 or tokens[1].lower() not in ["onsky", "internal"]:
                return make_control_reply(False, "Usage: mode onsky|internal", sim_control, runtime_status)
            sim_control["mode"] = tokens[1].lower()
            return make_control_reply(True, f"mode set to {sim_control['mode']}", sim_control, runtime_status)

        if cmd in ["phase", "ao", "scint"]:
            if len(tokens) != 2:
                return make_control_reply(False, f"Usage: {cmd} on|off", sim_control, runtime_status)
            key = f"{cmd}_enabled"
            sim_control[key] = bool_from_token(tokens[1])
            return make_control_reply(True, f"{key} set to {sim_control[key]}", sim_control, runtime_status)

        if cmd == "preset":
            if len(tokens) != 2:
                return make_control_reply(False, "Usage: preset onsky|internal|phase_only|scint_only", sim_control, runtime_status)
            ...
            if preset == "onsky":
                ...
                return make_control_reply(False, f"Unknown preset: {preset}", sim_control, runtime_status)
            return make_control_reply(True, f"preset set to {preset}", sim_control, runtime_status)
        ...
        if cmd == "set":
            if len(tokens) < 3:
                return make_control_reply(False, "Usage: set <parameter> <value>", sim_control, runtime_status)
            ...
            if key == "edge_offset_mm":
                ...
                return make_control_reply(False, f"Unknown set parameter: {key}", sim_control, runtime_status)

            return make_control_reply(True, f"set {key}", sim_control, runtime_status)

        return make_control_reply(False, f"Unknown command: {command}", sim_control, runtime_status)
...
while True:
    ...
    for ct, beam in enumerate(beams_shown ):
        ...
        if (
            ...
            opd_input_runtime, amp_input_runtime = generate_atmosphere_and_source_for_beam(
                zwfs=zwfs_ns[beam],
...
##############################################################################################################
# This version commented below tried to implement adding dynamic phasemask changing throug baldrapp/apps/paranal_simulator/fake_configs/phasemask_properties_physica_NOTWORKING.json
# which relies on an updated (still backwards compatible) spectrum.py script which is here
# These changes have not been commited 
##############################################################################################################
...
# def load_phasemask_properties(path):
#     """
#     Load fake/real phase-mask properties from JSON.

#     Preferred file format is a normal JSON object:
...
#     For convenience this also accepts a snippet beginning with:

#         "phasemask": {...}

#     and wraps it in braces before parsing.
#     """
#     path = Path(path)

#     if not path.exists():
#         print(f"[PHASEMASK] No phasemask properties file found: {path}", flush=True)
#         return {"default_mask": None, "masks": {}}
...
#         cfg = json.loads("{" + text.strip().strip(",") + "}")
...
#     print(
#         f"[PHASEMASK] Loaded {len(cfg['masks'])} mask definitions from {path}",
#         flush=True,
#     )
#     return cfg


# def install_physical_mask_diameter_runtime_patch():
#     """
#     Add support for optics.mask_diam_mode='physical_um' at runtime.

#     This is a compatibility shim for simulator deployments where
#     baldrapp.common.spectrum.phasemask_diameter_at_wavelength has not yet been
#     updated in the installed package. The better long-term fix is to put this
#     logic directly in baldrapp/common/spectrum.py.
#     """
#     if hasattr(spec, "_baldr_original_phasemask_diameter_at_wavelength"):
#         return

#     original = spec.phasemask_diameter_at_wavelength
#     spec._baldr_original_phasemask_diameter_at_wavelength = original

#     def _get_f_number_from_optics(optics):
#         if hasattr(optics, "F_number"):
#             return float(optics.F_number)
#         if hasattr(optics, "mask_Fratio"):
#             return float(optics.mask_Fratio)
#         if hasattr(optics, "f_number"):
#             return float(optics.f_number)
#         raise ValueError(
#             "Cannot convert physical phase-mask diameter: optics must contain "
#             "F_number, mask_Fratio, or f_number."
#         )

#     def patched_phasemask_diameter_at_wavelength(
#         optics,
#         wavelength_m,
#         default_wvl0=None,
#     ):
...
#             if hasattr(optics, "mask_diam_um"):
#                 mask_diam_m = float(optics.mask_diam_um) * 1e-6
#             elif hasattr(optics, "mask_diam_m"):
#                 mask_diam_m = float(optics.mask_diam_m)
#             else:
#                 raise ValueError(
#                     "optics.mask_diam_um or optics.mask_diam_m is required "
#                     "when mask_diam_mode='physical_um'."
#                 )
#             return mask_diam_m / (1.22 * _get_f_number_from_optics(optics) * wavelength_m)

#         if mode in ["physical_m", "m", "metre", "meter", "meters", "metres"]:
#             if not hasattr(optics, "mask_diam_m"):
#                 raise ValueError(
#                     "optics.mask_diam_m is required when mask_diam_mode='physical_m'."
#                 )
#             return float(optics.mask_diam_m) / (
#                 1.22 * _get_f_number_from_optics(optics) * wavelength_m
#             )

#         return original(optics, wavelength_m=wavelength_m, default_wvl0=default_wvl0)

#     spec.phasemask_diameter_at_wavelength = patched_phasemask_diameter_at_wavelength

#     # baldr_core imports the same module as bldr.spec, but set this explicitly
#     # for clarity and safety.
#     if hasattr(bldr, "spec"):
#         bldr.spec.phasemask_diameter_at_wavelength = patched_phasemask_diameter_at_wavelength

#     print("[PHASEMASK] Installed physical mask-diameter runtime support.", flush=True)


# def snapshot_optics_state(optics):
#     """Return a shallow copy of the current optics namespace state."""
#     return dict(vars(optics))


# def restore_optics_state(optics, state):
#     """
#     Restore optics fields to a previous snapshot and remove mask-specific fields
#     that were not present in the original JSON/default optics state.
#     """
#     mask_specific_keys = [
#         "current_mask",
#         "mask_depth",
...
# def get_mask_entry(phasemask_properties, mask_name):
#     """Return the dictionary entry for mask_name, or None if absent."""
#     masks = phasemask_properties.get("masks", {}) if phasemask_properties else {}
#     return masks.get(mask_name, None)


# def apply_phase_mask_entry_to_optics(
#     zwfs,
#     mask_name,
#     mask_entry,
#     verbose=False
# ):
#     """
#     Apply physical phase-mask parameters to zwfs.optics.

#     Supported mask-entry fields:
#         theta_mode
#         theta
#         mask_depth_um   -> converted to optics.mask_depth in metres
#         mask_depth      -> metres
#         dot_material
#         mask_diam_um    -> physical focal-plane diameter in microns
#         mask_diam_m     -> physical focal-plane diameter in metres
#         mask_diam       -> legacy dimensionless lambda/D-like diameter
#         mask_diam_mode  -> 'physical_um', 'physical_m', or legacy mode
#     """
#     optics = zwfs.optics
...
#     ]:
#         if key in entry:
#             setattr(optics, key, entry[key])

#     if "mask_depth_um" in entry:
#         optics.mask_depth = float(entry["mask_depth_um"]) * 1e-6
#         optics.mask_depth_um = float(entry["mask_depth_um"])

#     if "mask_diam_um" in entry:
#         optics.mask_diam_um = float(entry["mask_diam_um"])
#         optics.mask_diam_m = float(entry["mask_diam_um"]) * 1e-6
#         if "mask_diam_mode" not in entry:
#             optics.mask_diam_mode = "physical_um"

#     if "mask_diam_m" in entry:
#         optics.mask_diam_m = float(entry["mask_diam_m"])
#         if "mask_diam_mode" not in entry:
#             optics.mask_diam_mode = "physical_m"

#     optics.current_mask = str(mask_name)

#     # Update central-wavelength theta for diagnostics and monochromatic paths.
#     theta_mode = str(getattr(optics, "theta_mode", "constant")).lower()
#     if theta_mode == "physical_depth":
#         optics.theta = spec.theta_at_wavelength(optics, float(optics.wvl0))
#     elif theta_mode == "constant":
#         optics.theta = float(getattr(optics, "theta", 0.0))
#     else:
#         raise ValueError(f"Unsupported theta_mode for mask {mask_name}: {theta_mode}")

#     # Update central-wavelength legacy mask diameter for diagnostics and
#     # monochromatic paths. Polychromatic paths call this per wavelength.
#     optics.mask_diam = spec.phasemask_diameter_at_wavelength(
#         optics,
#         wavelength_m=float(optics.wvl0),
#         default_wvl0=float(optics.wvl0),
#     )

#     if verbose:
#         # print(
#         #     f"[PHASEMASK] {mask_name}: "
#         #     f"theta_mode={getattr(optics, 'theta_mode', None)}, "
#         #     f"theta(wvl0)={float(getattr(optics, 'theta', 0.0)):.6g}, "
#         #     f"mask_depth_um={getattr(optics, 'mask_depth_um', None)}, "
#         #     f"mask_diam_um={getattr(optics, 'mask_diam_um', None)}, "
#         #     f"mask_diam(lambda/D at wvl0)={float(getattr(optics, 'mask_diam', 0.0)):.6g}",
#         #     flush=True,
#         # )
#         print(
#             f"[PHASEMASK] active={mask_name} "
#             f"theta_mode={getattr(optics, 'theta_mode', None)} "
#             f"theta_wvl0={getattr(optics, 'theta', None)} "
#             f"mask_depth_m={getattr(optics, 'mask_depth', None)} "
#             f"dot_material={getattr(optics, 'dot_material', None)} "
#             f"mask_diam_mode={getattr(optics, 'mask_diam_mode', None)} "
#             f"mask_diam_um={getattr(optics, 'mask_diam_um', None)} "
#             f"mask_diam_lambdaD_wvl0={getattr(optics, 'mask_diam', None)} "
#             f"F_number={getattr(optics, 'F_number', None)}",
#             flush=True,
#         )
...
#     mask_name,
#     original_optics_state,
#     phasemask_properties=None,
#     verbose=False,
# ):
#     """
#     Apply the current fake-MDS phase-mask state to the optical model.

#     MDS convention:
#         fpm_whereami <beam> returns ""   -> mask out, theta=0
#         fpm_whereami <beam> returns "J3" -> apply J3 mask properties

#     If a named mask is not present in phasemask_properties, the simulator falls
#     back to the original inserted-mask optics for backwards compatibility.
#     """
#     mask_name = "" if mask_name is None else str(mask_name).strip()

#     # Start from the baseline JSON optics every time so stale fields from a
#     # previous mask do not carry over.
#     restore_optics_state(zwfs.optics, original_optics_state)
...
#     mask_entry = get_mask_entry(phasemask_properties, mask_name)

#     if mask_entry is None:
#         zwfs.optics.current_mask = mask_name
#         if verbose:
#             print(
#                 f"[PHASEMASK] No entry for {mask_name}; using default inserted-mask optics.",
#                 flush=True,
#             )
#         return True

#     apply_phase_mask_entry_to_optics(
#         zwfs=zwfs,
#         mask_name=mask_name,
...
# # Support physical phase-mask diameters in the active simulator process.
# # Long-term, keep the same logic in baldrapp/common/spectrum.py.
# install_physical_mask_diameter_runtime_patch()

# phasemask_properties_path = (
#     root_path
#     / "baldrapp/apps/paranal_simulator/fake_configs/phasemask_properties.json"
# )
# phasemask_properties = load_phasemask_properties(phasemask_properties_path)
...
#     original_optics_state[beam] = snapshot_optics_state(zwfs.optics)
...
#     default_mask = phasemask_properties.get("default_mask", "J3")
